...
class SamsungMultiroomDevice extends Homey.Device {
  ...
  rxDataFromSM(speakerData) {
    ...
    const spksInGroup = data['spksInGroup'].toInteger();
  ...
  groupPsOff() {
    ...
    const spksInGroup = state.activeGroupSpeakers.toInteger();
    const i = 1;
    while (i < spksInGroup) {
      const spkId = 'subSpk_${i}';
      i = i + 1;
  ...
  setGroupMasterVolume(masterVolume) {
    ...
    const groupData = state[preset_Data];
    const i = 1;
    const oldMastVol = device.currentValue('masterVolume').toInteger();
  ...
  setSubSpkVolume(mastVolIncrement) {
    const oldLevel = device.currentValue('level').toInteger();
  ...
  toggleGroupSpk() {
    const spksInGroup = state.activeGroupSpeakers.toInteger();
    const selSpkNo = state.selSpkNo.toInteger() + 1;
    const selSpkId = '';
    let selSpkVol;
    let selSpkEqLevel;
  ...
  //	==============================================
  playContent(preset) {
    const playerState = state[preset_Data];
    state.restore_Data = playerState;
    const contentType = playerState.type;
    const player = playerState.player;
    const playerNo = playerState.playerNo;
  ...
  setTrackDescription() {
    unschedule('setTrackDesciption');
    const submode = state.subMode;
    const source = device.currentValue('inputSource');
    if (source != 'wifi') {
  ...
  playTextAsVoiceAndResume(text, volume = null, voice = null) {
    ...
    if (state.spkType == 'sub') {
      ...
      const sound = textToSpeech(text, voice);
  ...
  playTrackAndResume(trackUrl, duration, volume = null) {
    const inputSource = device.currentValue('inputSource');
    const swType = getDataValue('swType');
    if (state.spkType == 'sub') {
      //	If a subspeaker in group, send to the Main speaker.
      ...
      if (state.resumePlay == '1') {
        const subMode = state.subMode;
        const oldLevel = device.currentValue('level').toInteger();
        const delayTime = duration.toInteger() + 2;
  ...
  async sendCmd(command) {
    ...
    this.log(command);
    const result = await fetch(`http://${address}:${PORT}/${command}`);
    ...
    switch (command) {
      ...
      case 'CurrentFunc':
        if (response.submode == 'dmr') {
          ...
          return;
        } else if (response.function != device.currentValue('inputSource') || response.submode != state.subMode) {
          //sendEvent(name: "inputSource", value: response.function)
          state.subMode = response.submode;
        }
        break;
      case 'VolumeLevel':
        volume = parseInt((100 * response.volume) / this.getData('volumeScale'));
      ...
      case 'MuteStatus':
        ...
        break;
      case '7BandEQList':
        cmdEqPreset(response.listcount);
        break;
      case 'EQMode':
      case 'EQDrc':
        GetCurrentEQMode();
      ...
      case 'CurrentEQMode':
        ...
        state.currentEqPreset = response.presetindex.toInteger();
        break;
      case 'RearLevel':
      ...
      case 'PlaybackStatus':
        let playerStatus;
        const prevStatus = device.currentValue('status');
      ...
      case 'MusicInfo':
        const timeLength = response.timelength.toString();
      ...
      case 'RadioInfo':
        ...
        if (response.tracklength == '' || response.tracklength == '0') {
          ...
          state.trackLength = response.tracklength.toInteger();
          state.updateTrackDescription = 'yes';
        }
        if (player == 'Unknown') {
        ...
        this.log(`generalResponse_${command}: Track Description is ${device.currentValue('trackDescription')}.`);
        getPlayTime();
        break;
      case 'MusicPlayTime':
        if (response.playtime != '' && response.playtime != null) {
          ...
          this.log(`generalResponse_${command}: Null playtime ignored. schedUpdateTrackDescription not called.`);
        }
        break;
      //	----- PLAY PRESET METHODS
      case 'CpChanged':
        ...
        if (player == 'AmazonPrime') {
          ...
          PlayById(playerState.player, playerState.path);
      ...
      case 'ChVolMultich':
        ...
        break;
      case 'AcmMode':
        const deviceMac = getDataValue('deviceMac').toString();
        const sourceMac = respData.audiosourcemacaddr.toString();
