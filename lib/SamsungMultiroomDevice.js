...
class SamsungMultiroomDevice extends Homey.Device {
  ...
  onInit() {
    ...
    this.registerCapabilityListener('volume', this.onCapabilityVolumeSet.bind(this));
    this.registerCapabilityListener('mute', this.onCapabilityMute.bind(this));
    this.registerCapabilityListener('preset', this.onCapabilityEQPreset.bind(this));
    this.registerCapabilityListener('eqPreset', this.onCapabilityEQPreset.bind(this));
    this.registerCapabilityListener('repeat', this.onCapabilityRepeat.bind(this));
    this.registerCapabilityListener('shuffle', this.onCapabilityShuffle.bind(this));
    this.registerCapabilityListener('trackDescription', this.onCapabilityTrackdescription.bind(this));
    this.registerCapabilityListener('status', this.onCapabilityStatus.bind(this));
    this.registerCapabilityListener('selSpkName', this.onCapabilityShuffle.bind(this));
    this.registerCapabilityListener('groupPsTitle', this.onCapabilityGroupPreset.bind(this));
    this.registerCapabilityListener('selSpkVol', this.onCapabilityStatus.bind(this));
    this.registerCapabilityListener('selSpkEqLevel', this.onCapabilityStatus.bind(this));
  ...
  async onCapabilityStatus() {}

  async onCapabilityTrackdescription() {}
  ...
  armAddPreset(preset) {
    ...
    this.runIn(15, cancelPresetUpdate, preset);
  }
  ...
  setTrackDescription() {
    ...
    if (source !== SOURCES.WIFI) {
      ...
      this.emit('shuffle', STATUS.INACTIVE);
      this.emit('repeat', STATUS.INACTIVE);
    } else {
      switch (submode) {
        case MODE.DLNA:
    ...
    switch (action) {
      ...
      case 'CurrentFunc':
        if (response.submode === 'dmr') {
          ...
          this.setCapabilityValue('on', true);
          this.emit('inputSource', response.function);
      ...
      case '7bandEQMode':
      case 'CurrentEQMode':
        this.emit('eqPreset', response.presetname);
      ...
      case 'RepeatMode':
        ...
        if (submode === MODE.DLNA) {
          ...
          this.emit('repeat', response.repeatmode);
        }
        break;
      case 'ShuffleMode':
        ...
        break;
      case 'ToggleShuffle':
        this.emit('shuffle', response.shufflemode);
        break;
      ...
      case 'RadioInfo':
        ...
        if (cpname === 'Unknown') {
          ...
          this.emit('trackDescription', '${respData.title}');
        }
